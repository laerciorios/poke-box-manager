...
The `BoxNavigation` SHALL support keyboard arrow keys for navigation.

#### Scenario: Left arrow navigates to previous box
- **WHEN** the navigation component has focus and the user presses the Left arrow key
- **THEN** the `onPrevious` callback SHALL be called (if not at first box)

#### Scenario: Right arrow navigates to next box
- **WHEN** the navigation component has focus and the user presses the Right arrow key
