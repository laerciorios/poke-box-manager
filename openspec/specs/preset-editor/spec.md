...
The system SHALL provide a `PresetEditor` component rendered as a Dialog with:
- A preset name text input (required, max 60 characters)
- A "Start from built-in" Select to seed rules from an existing preset
- A list of `RuleRow` components, one per rule
- An "Add Rule" button
- "Preview", "Save", and "Cancel" actions

#### Scenario: Open editor for new preset
- **WHEN** the user clicks "New Preset"
- **THEN** the Dialog SHALL open with an empty name and one empty rule

#### Scenario: Open editor to customize a built-in preset
- **WHEN** the user clicks "Customize" on a built-in preset card
- **THEN** the Dialog SHALL open pre-filled with that preset's name (suffixed with " (Custom)") and a copy of its rules

#### Scenario: Open editor to edit an existing custom preset
- **WHEN** the user clicks "Edit" on a custom preset card
- **THEN** the Dialog SHALL open pre-filled with that preset's current name and rules
...
- **THEN** the dialog SHALL close and the new preset SHALL appear in "My Presets"
